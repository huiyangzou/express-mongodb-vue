<template>
    <div style="">
        ...
        <div style="flex-direction: row;justify-content: space-between;display: flex;margin-top: 20px;margin-bottom: 20px;">
            <el-button type="primary" icon="el-icon-plus" @click="addGame">添加游戏</el-button>
        </div>
            ...
            <el-tabs v-model="activeName" type="card" @tab-click="handleClick">
                <el-tab-pane label="基本信息" name="first">
                    <div style="display: flex;flex-direction: row;align-items: center;margin-top: 20px;">
                        <span style="flex: 1;text-align: center">游戏名称</span>
                        <el-input style="flex:4;margin-left: 20px;" v-model="formData.gameName"
                                  placeholder="请输入内容"></el-input>
                    </div>

                    <div style="display: flex;flex-direction: row;align-items: center;margin-top: 20px;">
                        <span style="flex: 1;text-align: center">游戏链接</span>
                        <el-input style="flex:4;margin-left: 20px;" type="text"
                                   v-model="formData.gameLink"
                                  placeholder="请输入内容"></el-input>
                    </div>
                    <div style="display: flex;flex-direction: row;align-items: center;margin-top: 20px;">
                        <span style="flex: 1;text-align: center">游戏图片</span>
                        <el-input style="flex:4;margin-left: 20px;" type="text"
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            //获取列表数据
            getData() {
                this.$fetch('/v1/game', this.queryData)
                    .then((response) => {
            ...
            submit() {
                ...
                if (_.isEmpty(this.formData._id)) {
                    this.$post('/v1/game', this.formData)
                        .then((response) => {
                            console.log(response)
                            this.getData();
                        })
                } else {
                    this.$put('/v1/game/' + this.formData._id, this.formData)
                        .then((response) => {
                            console.log(response)
                            this.getData();
                        })
            ...
            handleClick(tab, event) {
                ...
                this.$fetch('/v1/game', {_id: tab._id})
                    .then((response) => {
                        console.log(response)
