...
const log = logger.getLogger('OrdersModule');
...
const OrdersModel = {
    ...
    update: async function(conditions, doc){
        try {
            await ordersModel.update(conditions, doc);
            return 'ok'
        } catch (error) {
    ...
    delete: async function(conditions){
        try {
            await ordersModel.deleteMany(conditions);
            return 'ok'
        } catch (error) {
    ...
    findOne: async function(conditions){
        try {
            return await ordersModel.findOne(conditions);
        } catch (error) {
            log.warn('delete error,', error);
            throw {status: 500, code: '1401', message: 'database error'};
        }
    ...
    findMany: async function(conditions){
        try {
            return await ordersModel.find(conditions);
        } catch (error) {
