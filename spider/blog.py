...
def getAllPage(soup):
    ...
    warp_dom = soup.find_all('div', attrs={'class':'post-preview'})

    for item in warp_dom:
        title = item.find('a').text
        url = item.find('a')['href']
        allPage = {'articleName':title,'articleLink':url}
        print(f"信息：{allPage}")
        r_json = requests.post("https://www.93goodtea.com/v1/article",allPage)
...
def main():
    for mark in range(33845,33846):
        ...
        print(f"标题：{htmlMark}")
        try:
            html = getHtml(f"http://gityuan.com/archive")
            print(f"html：{html}")
            soup = getSoup(html)
