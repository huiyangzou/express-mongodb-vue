...
const category = require('../bussiness/category');
const logger = require('../common/logger').logger;
const log = logger.getLogger('API');
var _ = require('lodash');

//查询
router.get('/v1/category', async function (req, res) {
    try {
        ...
        let result = await category.list(param);
...
//新增
router.post('/v1/category', async function (req, res) {
    try {
        let body = req.body;
...
//修改
router.put('/v1/category/:id', async function (req, res) {
    try {
...
//删除
router.delete('/v1/category/:id', async function (req, res) {
    try {
        let id = req.params.id;
