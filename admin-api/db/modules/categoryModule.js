...
const CategoryModel = {
    ...
    update: async function(conditions, doc){
        try {
            await categoryModel.update(conditions, doc);
            return 'ok'
        } catch (error) {
    ...
    delete: async function(conditions){
        try {
            await categoryModel.deleteMany(conditions);
            return 'ok'
        } catch (error) {
    ...
    findOne: async function(conditions){
        try {
            return await categoryModel.findOne(conditions);
        } catch (error) {
            log.warn('delete error,', error);
            throw {status: 500, code: '1401', message: 'database error'};
        }
    ...
    findMany: async function(conditions){
        try {
            ...
            const data = await categoryModel.find(conditions).skip(skip).limit(pageSize);
