...
const likevideoplayerModel = require('../models/likevideoplayerModel');

const LikevideoplayerModel = {

    create: async function(data){
        try {
            await likevideoplayerModel.create(data);
            return 'ok';
        } catch (error) {
    ...
    update: async function(conditions, doc){
        try {
            await likevideoplayerModel.update(conditions, doc);
            return 'ok'
        } catch (error) {
    ...
    delete: async function(conditions){
        try {
            await likevideoplayerModel.deleteMany(conditions);
            return 'ok'
        } catch (error) {
    ...
    findOne: async function(conditions){
        try {
            return await likevideoplayerModel.findOne(conditions);
        } catch (error) {
            log.warn('delete error,', error);
            throw {status: 500, code: '1401', message: 'database error'};
        }
    ...
    findMany: async function(conditions){
        try {
            ...
            let count=await likevideoplayerModel.countDocuments(conditions);
            const data = await likevideoplayerModel.find(conditions).skip(skip).limit(pageSize);
...
module.exports = LikevideoplayerModel;
