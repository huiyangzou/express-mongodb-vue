...
const GameModel = {
    ...
    update: async function(conditions, doc){
        try {
            await gameModel.update(conditions, doc);
            return 'ok'
        } catch (error) {
    ...
    delete: async function(conditions){
        try {
            await gameModel.deleteMany(conditions);
            return 'ok'
        } catch (error) {
    ...
    findOne: async function(conditions){
        try {
            return await gameModel.findOne(conditions);
        } catch (error) {
            log.warn('delete error,', error);
            throw {status: 500, code: '1401', message: 'database error'};
        }
    ...
    findMany: async function(conditions){
        try {
            ...
            let count=await gameModel.countDocuments(conditions);
            const data = await gameModel.find(conditions).skip(skip).limit(pageSize);
