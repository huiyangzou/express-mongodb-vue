...
const codeGeneratorModel = require('../models/codeGeneratorModel');

const CodeGeneratorModel = {

    create: async function(data){
        try {
            await codeGeneratorModel.create(data);
            return 'ok';
        } catch (error) {
    ...
    update: async function(conditions, doc){
        try {
            await codeGeneratorModel.update(conditions, doc);
            return 'ok'
        } catch (error) {
    ...
    delete: async function(conditions){
        try {
            await codeGeneratorModel.deleteMany(conditions);
            return 'ok'
        } catch (error) {
    ...
    findOne: async function(conditions){
        try {
            return await codeGeneratorModel.findOne(conditions);
        } catch (error) {
            log.warn('delete error,', error);
            throw {status: 500, code: '1401', message: 'database error'};
        }
    ...
    findMany: async function(conditions){
        try {
            ...
            let count=await codeGeneratorModel.countDocuments(conditions);
            const data = await codeGeneratorModel.find(conditions).skip(skip).limit(pageSize);
...
module.exports = CodeGeneratorModel;
