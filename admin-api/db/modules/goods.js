...
const GoodsModel = {
    ...
    update: async function(conditions, doc){
        try {
            await goodsModel.update(conditions, doc);
            return 'ok'
        } catch (error) {
    ...
    delete: async function(conditions){
        try {
            await goodsModel.deleteMany(conditions);
            return 'ok'
        } catch (error) {
    ...
    findOne: async function(conditions){
        try {
            return await goodsModel.findOne(conditions);
        } catch (error) {
            log.warn('delete error,', error);
            throw {status: 500, code: '1401', message: 'database error'};
        }
    ...
    findMany: async function(conditions){
        try {
            ...
            let count=await goodsModel.countDocuments(conditions);
            const data = await goodsModel.find(conditions).skip(skip).limit(pageSize);
