...
const csdnblogModel = require('../models/csdnblogModel');

const CsdnblogModel = {

    create: async function(data){
        try {
            await csdnblogModel.create(data);
            return 'ok';
        } catch (error) {
    ...
    update: async function(conditions, doc){
        try {
            await csdnblogModel.update(conditions, doc);
            return 'ok'
        } catch (error) {
    ...
    delete: async function(conditions){
        try {
            await csdnblogModel.deleteMany(conditions);
            return 'ok'
        } catch (error) {
    ...
    findOne: async function(conditions){
        try {
            return await csdnblogModel.findOne(conditions);
        } catch (error) {
            log.warn('delete error,', error);
            throw {status: 500, code: '1401', message: 'database error'};
        }
    ...
    findMany: async function(conditions){
        try {
            ...
            let count=await csdnblogModel.countDocuments(conditions);
            const data = await csdnblogModel.find(conditions).skip(skip).limit(pageSize);
