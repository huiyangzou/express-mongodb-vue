...
const HelloModel = {
    ...
    update: async function(conditions, doc){
        try {
            await helloModel.update(conditions, doc);
            return 'ok'
        } catch (error) {
    ...
    delete: async function(conditions){
        try {
            await helloModel.deleteMany(conditions);
            return 'ok'
        } catch (error) {
    ...
    findOne: async function(conditions){
        try {
            return await helloModel.findOne(conditions);
        } catch (error) {
            log.warn('delete error,', error);
            throw {status: 500, code: '1401', message: 'database error'};
        }
    ...
    findMany: async function(conditions){
        try {
            ...
            let count=await helloModel.countDocuments(conditions);
            const data = await helloModel.find(conditions).skip(skip).limit(pageSize);
