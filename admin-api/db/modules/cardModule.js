...
const CardModel = {
    ...
    update: async function(conditions, doc){
        try {
            await cardModel.update(conditions, doc);
            return 'ok'
        } catch (error) {
    ...
    delete: async function(conditions){
        try {
            await cardModel.deleteMany(conditions);
            return 'ok'
        } catch (error) {
    ...
    findOne: async function(conditions){
        try {
            return await cardModel.findOne(conditions);
        } catch (error) {
            log.warn('delete error,', error);
            throw {status: 500, code: '1401', message: 'database error'};
        }
    ...
    findMany: async function(conditions){
        try {
            ...
            let count=await cardModel.countDocuments(conditions);
            const data = await cardModel.find(conditions).skip(skip).limit(pageSize);
