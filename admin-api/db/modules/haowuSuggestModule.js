...
const haowuSuggestModel = require('../models/haowuSuggestModel');

const HaowuSuggestModel = {

    create: async function(data){
        try {
            await haowuSuggestModel.create(data);
            return 'ok';
        } catch (error) {
    ...
    update: async function(conditions, doc){
        try {
            await haowuSuggestModel.update(conditions, doc);
            return 'ok'
        } catch (error) {
    ...
    delete: async function(conditions){
        try {
            await haowuSuggestModel.deleteMany(conditions);
            return 'ok'
        } catch (error) {
    ...
    findOne: async function(conditions){
        try {
            return await haowuSuggestModel.findOne(conditions);
        } catch (error) {
            log.warn('delete error,', error);
            throw {status: 500, code: '1401', message: 'database error'};
        }
    ...
    findMany: async function(conditions){
        try {
            ...
            let count=await haowuSuggestModel.countDocuments(conditions);
            const data = await haowuSuggestModel.find(conditions).skip(skip).limit(pageSize);
...
module.exports = HaowuSuggestModel;
